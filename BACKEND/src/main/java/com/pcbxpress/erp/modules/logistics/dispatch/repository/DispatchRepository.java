...
public interface DispatchRepository extends JpaRepository<Dispatch, UUID> {
    ...
    @Query("SELECT d FROM Dispatch d WHERE " +
           ...
           "(:warehouseId IS NULL OR d.warehouseId = :warehouseId) AND " +
           "(:carrierId IS NULL OR d.carrierId = :carrierId) AND " +
           "(:status IS NULL OR d.status = :status) AND " +
           "(:priority IS NULL OR d.priority = :priority) AND " +
           "(:dispatchType IS NULL OR d.dispatchType = :dispatchType) AND " +
           "(:query IS NULL OR LOWER(d.code) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "OR LOWER(d.description) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "OR LOWER(d.trackingNumber) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "OR LOWER(d.customerName) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "OR LOWER(d.carrierName) LIKE LOWER(CONCAT('%', :query, '%')))")
    List<Dispatch> findByCriteria(
        ...
        @Param("priority") Dispatch.Priority priority,
        @Param("dispatchType") Dispatch.DispatchType dispatchType,
    ...
    List<Dispatch> findByWeightRangeAndStatus(@Param("minWeight") Double minWeight, @Param("maxWeight") Double maxWeight,
                                              @Param("status") Dispatch.DispatchStatus status);
    ...
    @Query("SELECT d FROM Dispatch d WHERE d.dimensionsLength >= :minLength AND d.dimensionsLength <= :maxLength " +
           ...
           "AND d.dimensionsHeight >= :minHeight AND d.dimensionsHeight <= :maxHeight " +
           "AND d.status = :status")
    List<Dispatch> findByDimensionsRangeAndStatus(@Param("minLength") Double minLength, @Param("maxLength") Double maxLength,
                                                  @Param("minWidth") Double minWidth, @Param("maxWidth") Double maxWidth,
                                                  @Param("minHeight") Double minHeight, @Param("maxHeight") Double maxHeight,
                                                  @Param("status") Dispatch.DispatchStatus status);
    ...
    @Query("SELECT d FROM Dispatch d WHERE d.weight >= :minWeight AND d.weight <= :maxWeight")
    Page<Dispatch> findDispatchesByWeightRange(@Param("minWeight") Double minWeight, @Param("maxWeight") Double maxWeight,
                                               Pageable pageable);
    ...
    @Query("SELECT d FROM Dispatch d WHERE d.dimensionsLength >= :minLength AND d.dimensionsLength <= :maxLength " +
           "AND d.dimensionsWidth >= :minWidth AND d.dimensionsWidth <= :maxWidth " +
           "AND d.dimensionsHeight >= :minHeight AND d.dimensionsHeight <= :maxHeight")
    Page<Dispatch> findDispatchesByDimensionsRange(@Param("minLength") Double minLength, @Param("maxLength") Double maxLength,
                                                   @Param("minWidth") Double minWidth, @Param("maxWidth") Double maxWidth,
                                                   @Param("minHeight") Double minHeight, @Param("maxHeight") Double maxHeight,
                                                   Pageable pageable);
}