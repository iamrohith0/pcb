...
public class DispatchService {
    ...
    public List<DispatchDto> list(String query, UUID orderId, UUID customerId, UUID warehouseId,
        ...
        return dispatchRepository
                ...
                .sorted(Comparator.comparing(Dispatch::getUpdatedAt, Comparator.nullsLast(Comparator.naturalOrder()))
                        .reversed())
                .map(this::toDto)
    ...
    public DispatchDto create(DispatchPayload payload) {
        ...
        // NOTE: Optional numeric fields (weight, value, dimensions) are NOT defaulted
        // to ZERO
        // Leaving them NULL preserves data integrity - distinguishes "not entered" from
        // "zero"

        // SAFE: Run validation to ensure required fields are present (throws if
        // missing)
        validateAndNormalizeBeforeSave(dispatch);
     ...
     */
    public List<DispatchDto> getDispatchHistory(UUID orderId, String status, OffsetDateTime startDate,
        ...
        return dispatches.stream()
                .sorted(Comparator.comparing(Dispatch::getDispatchDate, Comparator.nullsLast(Comparator.naturalOrder()))
                        .reversed())
                .map(this::toDto)
                .collect(Collectors.toList());
    }
     ...
     */
    public List<DispatchDto> getUpcomingDispatches(OffsetDateTime startDate, OffsetDateTime endDate,
        ...
        return dispatches.stream()
                .sorted(Comparator.comparing(Dispatch::getEstimatedDelivery,
    ...
    private void applyPayload(Dispatch target, DispatchPayload payload) {
        // Apply payload fields with null-safety
        if (payload.code() != null && !payload.code().isBlank()) {
        ...
        // Order context - use payload or keep existing
        ...
        // Customer context - use payload or keep existing
        ...
        // Warehouse context - use payload or keep existing
        ...
        // Carrier info - safe to be null
        if (payload.carrierId() != null) {
            target.setCarrierId(payload.carrierId());
        }
        if (payload.carrierName() != null) {
        ...
        if (payload.receivedBy() != null) {
            target.setReceivedBy(payload.receivedBy());
        }
    }

    /**
     * STRICT VALIDATION - enforces required fields before save.
     * 
     * RULES:
     * 1. REQUIRED UUID fields (orderId, customerId, warehouseId) → throw
     * ValidationException if null
     * (NO placeholder UUIDs - they cause FK violations and data integrity issues)
     * 2. OPTIONAL fields → preserve NULL, do NOT overwrite with placeholders
     * 3. ENUM defaults (status, priority, dispatchType) → apply ONLY if truly null
     * 4. NEVER overwrite user-provided values
     * 
     * This prevents:
     * - HTTP 500 errors from FK violations
     * - Silent data corruption with fake UUIDs
     * - "UNKNOWN"/"DEFAULT" placeholder values in database
     */
    private void validateAndNormalizeBeforeSave(Dispatch dispatch) {
        // === REQUIRED UUID FIELDS (nullable=false in schema) ===
        // These MUST be validated - throw DispatchException if missing
        // This returns HTTP 400 with clear error message to frontend

        if (dispatch.getOrderId() == null) {
            throw new DispatchException.ValidationException(
                    "Order ID is required. Please select a valid order before saving.");
        }
        if (dispatch.getCustomerId() == null) {
            throw new DispatchException.ValidationException(
                    "Customer ID is required. Please select a valid customer before saving.");
        }
        if (dispatch.getWarehouseId() == null) {
            throw new DispatchException.ValidationException(
                    "Warehouse ID is required. Please select a valid warehouse before saving.");
        }

        // === ENUM DEFAULTS - apply ONLY if truly null ===
        // These are safe system defaults that don't corrupt user data
        ...
        // === OPTIONAL FIELDS - DO NOT DEFAULT ===
        // customerName, orderCode, warehouseName: leave NULL if not provided
        // weight, value, dimensions: leave NULL if not provided
        // weightUnit, currency: leave NULL if not provided
        // dispatchDate: can be NULL for DRAFT status
    }
    ...
    private void normalizeBeforeSave_DEPRECATED(Dispatch dispatch) {
        // REMOVED: Placeholder UUID logic that caused data corruption
        // REMOVED: "UNKNOWN"/"UNLINKED"/"DEFAULT" text overwrites
        // REMOVED: BigDecimal.ZERO numeric overwrites
        // See validateAndNormalizeBeforeSave() for the correct approach
    }
    ...
    private static String getDimensionsString(DispatchDto dispatch) {
        if (dispatch.dimensionsLength() == null || dispatch.dimensionsWidth() == null
                || dispatch.dimensionsHeight() == null) {
            return "";
        }
