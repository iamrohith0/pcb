...
public class TrackingService {
    ...
    public List<TrackingDto> list(String query, UUID shipmentId, UUID packageId, UUID carrierId, 
        ...
        return trackingRepository.findByCriteria(shipmentId, packageId, carrierId, status, trackingType, isActive, query).stream()
            .filter(tracking -> {
            ...
            .sorted(Comparator.comparing(Tracking::getUpdatedAt, Comparator.nullsLast(Comparator.naturalOrder())).reversed())
            .map(this::toDto)
    ...
    public Page<TrackingDto> listWithPagination(String query, UUID shipmentId, UUID packageId, UUID carrierId, 
        ...
        if (startDate != null && endDate != null) {
            ...
            List<Tracking> allTrackings = trackingRepository.findByCriteria(shipmentId, packageId, carrierId, status, trackingType, isActive, query);
    ...
    public List<TrackingDto> getByShipment(UUID shipmentId) {
        return trackingRepository.findByShipmentId(shipmentId).stream()
            .sorted(Comparator.comparing(Tracking::getEventTime, Comparator.nullsLast(Comparator.naturalOrder())).reversed())
            .map(this::toDto)
            .collect(Collectors.toList());
    }
     ...
     */
    public List<TrackingDto> getTrackingHistory(String trackingId, String status, OffsetDateTime startDate, OffsetDateTime endDate) {
        List<Tracking> trackings = trackingRepository.findByTrackingId(trackingId);
    ...
    public Map<String, Object> getTrackingReport(String status, OffsetDateTime startDate, OffsetDateTime endDate) {
        ...
        long outForDelivery = all.stream().filter(t -> t.getStatus() == Tracking.TrackingStatus.OUT_FOR_DELIVERY).count();
        long delivered = all.stream().filter(t -> t.getStatus() == Tracking.TrackingStatus.DELIVERED).count();
        long failedDelivery = all.stream().filter(t -> t.getStatus() == Tracking.TrackingStatus.FAILED_DELIVERY).count();
        ...
        long availableForPickup = all.stream().filter(t -> t.getStatus() == Tracking.TrackingStatus.AVAILABLE_FOR_PICKUP).count();
    ...
    private void validateUniqueConstraints(TrackingPayload payload, String existingId) {
        ...
        if (payload.trackingId() != null && !payload.trackingId().isBlank()) {
            boolean exists = trackingRepository.existsByTrackingIdIgnoreCaseAndIdNot(payload.trackingId(), existingUUID);
            if (exists) {
    ...
    private static String getDeliveryContactString(TrackingDto tracking) {
        if (tracking.deliveryContactName() == null && tracking.deliveryContactPhone() == null && tracking.deliveryContactEmail() == null) {
            return "";
        }
