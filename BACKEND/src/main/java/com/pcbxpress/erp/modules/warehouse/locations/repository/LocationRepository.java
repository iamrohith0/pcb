...
public interface LocationRepository extends JpaRepository<Location, UUID> {
       ...
       @Query("SELECT l FROM Location l WHERE " +
                     ...
                     "AND (:shelf IS NULL OR (l.shelf IS NOT NULL AND LOWER(l.shelf) = LOWER(:shelf))) " +
                     "AND (:bin IS NULL OR (l.bin IS NOT NULL AND LOWER(l.bin) = LOWER(:bin))) " +
                     "AND (:searchText IS NULL OR :searchText = '' OR " +
                     "(l.name IS NOT NULL AND LOWER(l.name) LIKE LOWER(CONCAT('%', :searchText, '%'))) OR " +
                     "(l.description IS NOT NULL AND LOWER(l.description) LIKE LOWER(CONCAT('%', :searchText, '%'))) OR "
                     +
                     "(l.code IS NOT NULL AND LOWER(l.code) LIKE LOWER(CONCAT('%', :searchText, '%'))) OR " +
                     "(l.warehouseName IS NOT NULL AND LOWER(l.warehouseName) LIKE LOWER(CONCAT('%', :searchText, '%'))))")
