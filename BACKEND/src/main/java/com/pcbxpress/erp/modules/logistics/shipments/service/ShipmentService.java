...
public class ShipmentService {
    ...
    public List<ShipmentDto> list(String query, UUID orderId, UUID customerId, UUID warehouseId,
        ...
        return shipmentRepository
                .findByCriteria(orderId, customerId, warehouseId, carrierId, status, priority, shipmentType,
                        serviceLevel, query)
                .stream()
                .filter(shipment -> {
                ...
                .sorted(Comparator.comparing(Shipment::getUpdatedAt, Comparator.nullsLast(Comparator.naturalOrder()))
                        .reversed())
                .map(this::toDto)
    ...
    public Page<ShipmentDto> listWithPagination(String query, UUID orderId, UUID customerId, UUID warehouseId,
        ...
        if (startDate != null && endDate != null) {
            ...
            List<Shipment> allShipments = shipmentRepository.findByCriteria(orderId, customerId, warehouseId, carrierId,
                    status, priority, shipmentType, serviceLevel, query);
    ...
    public ShipmentDto create(ShipmentPayload payload) {
        ...
        if (payload.warehouseId() == null)
            missing.add("warehouseId");
        if (payload.carrierId() == null)
            missing.add("carrierId");
        if (!missing.isEmpty()) {
     ...
     */
    public List<ShipmentDto> getShipmentHistory(UUID orderId, String status, OffsetDateTime startDate,
        ...
        return shipments.stream()
                .sorted(Comparator.comparing(Shipment::getShipmentDate, Comparator.nullsLast(Comparator.naturalOrder()))
                        .reversed())
                .map(this::toDto)
                .collect(Collectors.toList());
    }
    ...
    public Map<String, Object> getShipmentReport(String status, OffsetDateTime startDate, OffsetDateTime endDate) {
        ...
        long international = all.stream().filter(s -> s.getShipmentType() == Shipment.ShipmentType.INTERNATIONAL)
                .count();
     ...
     */
    public List<ShipmentDto> getUpcomingShipments(OffsetDateTime startDate, OffsetDateTime endDate,
        ...
        return shipments.stream()
                .sorted(Comparator.comparing(Shipment::getEstimatedDelivery,
